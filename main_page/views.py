...
def main_page(request):
    ...
    if is_logged:
        return render(request, 'main_page/index.html', context={'is_logged_key': True})
    else:
        return render(request, 'main_page/not_login_index.html', context={'is_logged_key': False})
...
def log_in(request):
    ...
    if is_logged:
        ...
        return render(request, 'registration/login.html', context={'is_logged_key': False, 'errors': errors})
...
def sign_in(request):
    ...
    if is_logged:
        ...
        errors = request.session.get('errors', False)
        request.session['errors'] = False
        return render(request, 'registration/signup.html', context={'is_logged_key': False, 'errors': errors})
    ...
    if is_logged:
        ...
        if request.method == 'POST':
            ...
            if len(errors) == 0:
                ...
                User.objects.create(username=username, email=email, password=password, salt=salt)
...
def me(request):
    ...
    if is_logged:
        ...
        return render(request, 'aboutMe/me.html', context={'is_logged_key': True, 'user': user})
    else:
...
def porady(request):
    ...
    is_logged = request.session.get('is_logged', False)
    if is_logged:
        return render(request, 'environment/porady.html', context={'is_logged_key': True})
    else:
        return redirect('log_out')
...
def credits(request):
    ...
    is_logged = request.session.get('is_logged', False)
    if is_logged:
        return render(request, 'credits.html', context={'is_logged_key': True})
    else:
        return redirect('log_out')
...
def http_not_found(request):
    ...
    is_logged = request.session.get('is_logged', False)
    if is_logged:
        return render(request, '404.html', context={'is_logged_key': True})
    else:
        return redirect('log_out')
...
def lekarze(request):
    ...
    is_logged = request.session.get('is_logged', False)
    if is_logged:
        return render(request, 'lekarze.html', context={'is_logged_key': True})
    else:
        return redirect('log_out')
...
def Calendar(request):
    ...
    is_logged = request.session.get('is_logged', False)
    if is_logged:
        return render(request, 'environment/Calendar.html', context={'is_logged_key': True})
    else:
        return redirect('log_out')
        ...
        if request.method == 'POST':
            ...
            if len(mood_data_list) != 0:
                for elem in mood_data_list:
                    ...
                    Post.objects.create(user_id=User.objects.get(id=user_id_s), content=elem["text"],
                ...
                _charts = os.path.join(os.path.abspath(os.curdir), "main_page", "static", "images", "auto-generated-charts")
                if not os.path.exists(_charts):
                    os.mkdir(_charts)

                _dir = os.path.join(_charts, "user-" + str(user_id_s))
                if not os.path.exists(_dir):
                    os.mkdir(_dir)

                plt.savefig(os.path.join(_dir, 'last_well_being.png'), format="png")
            else:
                pass
        else:
            pass
...
def wykresy(request):
    ...
    if is_logged:
        ...
        return render(request, 'post-related-pages/wykresy.html', context={'is_logged_key': True, 'posts': posts,
